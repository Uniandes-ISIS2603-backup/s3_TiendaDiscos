...
public class WishListLogic {
     ...
     * @param usuarioId El id del usuario el cual será el padre del nuevo
     * Wishlist.
     * @return Objeto de WishListEntity con los datos nuevos y su ID.
     * @throws BusinessLogicException si el usuario con el id usuarioId ya tiene
     * asignado un Wishlist. Si no existe un usuario con ese id.
     */
    ...
    public WishListEntity agregarVinilo(Long usuariosId, Long vinilosId) throws BusinessLogicException, BusinessLogicException {
        ...
        return wishlistPersitence.findByUserId(usuariosId);
    }
    ...
    public WishListEntity eliminarVinilo(Long usuariosId, Long vinilosId) throws BusinessLogicException {
      ...
      List<ViniloEntity> vinilos = wishListEntity.getVinilos();
        ...
        if (estaConetinido) {
            List<WishListEntity> wishLists = viniloEntity.getWishLists();
            for (int i = 0; i < wishLists.size() && posicion==-1; i++) {
                WishListEntity wishList1 = wishLists.get(i);
                if (wishList1.getId() == wishListEntity.getId() && wishList1.getCosto() == wishListEntity.getCosto()) {
                    posicion=i;
                }
            }
            viniloEntity.getWishLists().remove(posicion);
            wishListEntity.setCosto(wishListEntity.getCosto() - viniloEntity.getPrecio());
        } else {
            throw new BusinessLogicException("El vinilo no existe en la wishList");
